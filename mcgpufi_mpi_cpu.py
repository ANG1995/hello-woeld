...
if __name__ == "__main__":
  ...
  for ms_index,ms in enumerate(ms_names):
    ...
    chunk_size = int(np.ceil(data._no_rows / float(comm_size)))
    ...
    if comm_size < 2 :
       raise Exception("The MPI procee can not less than 2 !")

    chunk_index = comm_rank 
    ...
    logging.info("READING CHUNK %d OF %d" % (chunk_index+1,comm_size))
  ...
  '''
      ...
      for f in range(0, max(1, num_facet_centres)):
        image_prefix = parser_args['output_prefix'] if num_facet_centres == 0 else parser_args[
                                                                                     'output_prefix'] + "_facet" + str(f)
        if parser_args['output_format'] == 'png':
          ...
          if parser_args['output_psf']:
            for i, c in enumerate(channels_to_image):
              ...
              psf = np.ctypeslib.as_array(
                ctypes.cast(sampling_funct.ctypes.data + offset, ctypes.POINTER(ctypes.c_float)),
                shape=(npix_l, npix_m))
              psf /= np.max(psf)
          ...
          fits_export.save_to_fits_image(image_prefix + '_dirty.fits',
                                         parser_args['npix_l'], parser_args['npix_m'],
                                         quantity(parser_args['cell_l'], 'arcsec'),
                                         quantity(parser_args['cell_m'], 'arcsec'),
          ...
          if parser_args['output_psf']:
            ...
            psf = np.ctypeslib.as_array(
              ctypes.cast(sampling_funct.ctypes.data + offset_psf, ctypes.POINTER(ctypes.c_float)),
              shape=(cube_chan_dim_size, npix_l, npix_m))
            psf /= np.max(psf)
      ...
      logging.info(
        "\tConvolution filter and detapering function creation time: %f secs" % filter_creation_timer.elapsed())
      logging.info("\tIn Parallel:")
      logging.info(
        "\t\tData loading and conversion time: %f secs" % data_set_loader.data_set_loader.time_to_load_chunks.elapsed())
      ...
      time_log = '%s %f %f %f %f %f' % (
        nowTime, filter_creation_timer.elapsed(), data_set_loader.data_set_loader.time_to_load_chunks.elapsed(), \
