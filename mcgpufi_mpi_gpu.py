...
import mcgpufi_mo.library_loader as library_loader
from helpers import timer
from helpers import png_export
from helpers import fits_export
from helpers.deconvolution import deconvolve_cube
from helpers.deconvolution import restore_cube 

if __name__ == "__main__":
  logger = logging.getLogger('mcgpufi')
  ...
  from mcgpufi_mo import base_types
  base_types.force_precision(parser_args['precision'])
  from helpers import data_set_loader
  from mcgpufi_mo import convolution_filter
  from mcgpufi_mo import gridding_parameters
  ...
  '''
  ms = ms_names[comm_rank]
  logging.info("NOW IMAGING %s" % ms)
  ...
  logging.info("The maximum w (measured in wavelengths) is estimated to be {0}".format(w_max))  
  ...
  logging.info("IMAGING ONLY FIELD %s" % data._field_centre_names[parser_args['field_id']])
  ...
  if not parser_args['do_jones_corrections']:
      if gridded_vis == None:
        ...
        gridded_vis_sum = np.zeros([num_facet_grids,cube_chan_dim_size,len(correlations_to_grid),npix_l,npix_m],dtype=base_types.grid_type)
  else:
      if gridded_vis == None:
        gridded_vis = np.zeros([num_facet_grids,cube_chan_dim_size,4,npix_l,npix_m],dtype=base_types.grid_type)
        gridded_vis_sum = np.zeros([num_facet_grids,cube_chan_dim_size,4,npix_l,npix_m],dtype=base_types.grid_type)
  ...
  params.baseline_count = ctypes.c_size_t(int(data._no_baselines)) #this ensures a deep copy
  ...
  for chunk_index in range(0,no_chunks):
    ...
    logging.info("READING CHUNK %d OF %d" % (chunk_index+1,no_chunks))
    ...
    if parser_args['use_back_end'] == 'GPU':
      with data_set_loader.data_set_loader.time_to_load_chunks:
        starting_indexes = np.zeros([int(data._no_baselines)+1],dtype=np.intp) #this must be n(n-1)/2+n+1 since we want to be able to compute the number of timestamps for the last baseline
  ...
  logging.info("process %d gridding finish" % comm_rank)
  comm.Reduce(gridded_vis, gridded_vis_sum, op=MPI.SUM, root=0)
  ...
  if comm_rank == 0:
    if parser_args['pol'] in ["I","Q","U","V"]:
      for f in range(0,max(1,num_facet_centres)):
        for c in range(0,cube_chan_dim_size):
          ...
          nf = int(shift_count % len(correlations_to_grid))
          nc = int(shift_count / len(correlations_to_grid))
          gridded_vis[f,nc,nf,:,:] = gridded_vis_sum[f,c,0,:,:]
    ...
    '''
    #dirty_comb = np.zeros(shape=(cube_chan_dim_size,parser_args['n_facets_l']*parser_args['npix_l'],parser_args['n_facets_m']*parser_args['npix_m']),dtype=float) 
    #lf = 0  
    #mf = 0 
    ...
    for f in range(0, max(1,num_facet_centres)):
      ...
      if parser_args['output_format'] == 'png':
        ...
        fits_export.save_to_fits_image(image_prefix+'_dirty.fits',
				       parser_args['npix_l'],parser_args['npix_m'],
        ...
        if parser_args['output_psf']:
           offset_psf = npix_m*npix_l*f*np.dtype(np.float32).itemsize
           psf = np.ctypeslib.as_array(ctypes.cast(sampling_funct.ctypes.data + offset_psf, ctypes.POINTER(ctypes.c_float)),
					shape=(cube_chan_dim_size,npix_l,npix_m))
           psf /= np.max(psf)
           ...
           fits_export.save_to_fits_image(image_prefix+'_restored.fits',
				       restored.shape[1],restored.shape[2],
				       quantity(parser_args['cell_l'],'arcsec'),quantity(parser_args['cell_m'],'arcsec'),
				       centre_coord_l,centre_coord_m,
				       quantity(ra,'arcsec'),
				       quantity(dec,'arcsec'),
				       ...
				       cube_delta_wavelength,
				       cube_chan_dim_size,
				       restored[:,0:restored.shape[1],0:restored.shape[2]])

  #      mf = f % parser_args['n_facets_m'] 
  #      #print parser_args['n_facets_l'],parser_args['npix_l'],parser_args['n_facets_m'],parser_args['npix_l'] 
  #      dirty_comb[:,(mf*parser_args['npix_m']):((mf+1)*parser_args['npix_m']),(parser_args['n_facets_l']*parser_args['npix_l']-(lf+1)*parser_args['npix_l']):(parser_args['n_facets_l']*parser_args['npix_l']-(lf)*parser_args['npix_l'])]=dirty[:,l_left_margin:l_right_margin,m_left_margin:m_right_margin]  
  #
  # 
  #      if (f+1)%parser_args['n_facets_l'] == 0 : 
  #          lf = lf + 1 
        ...
        if parser_args['output_psf']:
          for i,c in enumerate(channels_to_image):
            ...
            psf /= np.max(psf)
            spw_no = int(c / data._no_channels)
            chan_no = int(c % data._no_channels)
            ra = data._field_centres[parser_args['field_id'],0,0]
            dec = data._field_centres[parser_args['field_id'],0,1]
            offset_coord_l = (0 if num_facet_centres == 0 else facet_centres[f,0] - ra) / parser_args['cell_l']
            centre_coord_l = (parser_args['npix_l'] * 0.5 + 1) + offset_coord_l
  ...
  #  ra = data._field_centres[parser_args['field_id'],0,0]
  #  dec = data._field_centres[parser_args['field_id'],0,1]
  #  offset_coord_l = (0 if num_facet_centres == 0 else ra) / parser_args['cell_l']
  #  centre_coord_l = (parser_args['npix_l'] * parser_args['n_facets_l'] * 0.5 + 1) + offset_coord_l
  #  offset_coord_m = (0 if num_facet_centres == 0 else dec) / parser_args['cell_m']
  #  centre_coord_m = (parser_args['npix_m'] * parser_args['n_facets_m'] * 0.5 + 1) - offset_coord_m
  #  fits_export.save_to_fits_image('facet_comb.fits', 
  #                                      parser_args['npix_l']*parser_args['n_facets_l'],parser_args['npix_m']*parser_args['n_facets_m'], 
  #                                      quantity(parser_args['cell_l'],'arcsec'),quantity(parser_args['cell_m'],'arcsec'), 
  #                                      centre_coord_l,centre_coord_m, 
  #                                      quantity(ra,'arcsec'), 
  #                                      quantity(dec,'arcsec'), 
  ...
  #                                      cube_delta_wavelength, 
  #                                      cube_chan_dim_size, 
  #                                      dirty_comb[:,0:parser_args['npix_l']*parser_args['n_facets_l'],0:parser_args['npix_m']*parser_args['n_facets_m']])
    ...
    if parser_args['stitch_facets']: #we've already checked that there are multiple facets before this line
      logging.info("ATTEMPTING TO MOSAIC FACETS USING SWarp (MUST BE IN ENV PATH)")
      comb_img_size = parser_args['npix_l'] * parser_args['n_facets_l']
      facet_list_parser.output_mosaic(parser_args['output_prefix'],num_facet_centres,comb_img_size)
      if parser_args['open_default_viewer']:
         os.system("xdg-open %s.combined.fits" % parser_args['output_prefix'])

    logging.info("FINISHED WORK SUCCESSFULLY")
    total_run_time.stop()
    logging.info("STATISTICS:")
    logging.info("\tConvolution filter and detapering function creation time: %f secs" % filter_creation_timer.elapsed())
    logging.info("\tIn Parallel:")
    logging.info("\t\tData loading and conversion time: %f secs" % data_set_loader.data_set_loader.time_to_load_chunks.elapsed())
    libimaging.get_gridding_walltime.restype = ctypes.c_double
    libimaging.get_inversion_walltime.restype = ctypes.c_double
    logging.info("\t\tGridding time: %f secs" % libimaging.get_gridding_walltime())
    logging.info("\tFourier inversion time: %f secs" % libimaging.get_inversion_walltime())
    logging.info("\tTotal runtime: %f secs" % total_run_time.elapsed())
    #Save run time data
    nowTime = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    time_log = '%s %f %f %f %f %f' % (nowTime, filter_creation_timer.elapsed(), data_set_loader.data_set_loader.time_to_load_chunks.elapsed(),\
				     libimaging.get_gridding_walltime(), libimaging.get_inversion_walltime(), total_run_time.elapsed())
    with open(parser_args['run_time_output'], "a") as f:
         f.write(time_log + "\n")
